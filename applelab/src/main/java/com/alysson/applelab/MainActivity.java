...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        sub.setText("G991B • Exynos 2100 • scanner OPP GPU/MIF somente leitura\nNão escreve clock. 900/3264 só são REAIS se aparecerem nas tabelas do kernel.");
    ...
    private void runFullScan(){
        busy(true); status.setText("Escaneando GPU, devfreq/MIF, CPU, UV e thermal...");
        worker.submit(()->{
    ...
    private String verdict(String r,int rc){
        ...
        boolean g=r.contains("GPU900_OPP_REAL=YES"), m=r.contains("MIF3264_OPP_REAL=YES");
        return "OK • GPU max="+value(r,"GPU_TABLE_MAX_KHZ")+" kHz • 900="+(g?"REAL":"não")+" • MIF max="+value(r,"MIF_TABLE_MAX_KHZ")+" kHz • 3264="+(m?"REAL":"não");
    }
    ...
    private static final String SCAN_SCRIPT=String.join("\n",
        ...
        "find /sys/devices /sys/kernel /sys/class -maxdepth 8 -type f -name dvfs_table 2>/dev/null | head -40 | while read f; do dirname \"$f\"; done >> $TMP",
        "find /sys/devices /sys/kernel /sys/class -maxdepth 8 -type f -name asv_table 2>/dev/null | head -40 | while read f; do dirname \"$f\"; done >> $TMP",
        ...
        "if [ -n \"$MIF\" ]; then AV=$(cat \"$MIF/available_frequencies\" 2>/dev/null); echo MIF_REALPATH=$(readlink -f \"$MIF\" 2>/dev/null); echo MIF_AVAILABLE_FREQUENCIES=$AV; for x in $(printf '%s' \"$AV\" | tr -cs '0-9' ' '); do [ \"$x\" = 3264000 ] && M3264=YES; [ \"$x\" -gt $MMAX ] 2>/dev/null && MMAX=$x; done; [ $MMAX -eq 0 ] && MMAX=$(cat \"$MIF/max_freq\" 2>/dev/null); echo MIF_CUR_KHZ=$(cat \"$MIF/cur_freq\" 2>/dev/null); echo MIF_MIN_KHZ=$(cat \"$MIF/min_freq\" 2>/dev/null); echo MIF_MAX_KHZ=$(cat \"$MIF/max_freq\" 2>/dev/null); fi",
        ...
        "echo '=== MALI MODULES ==='",
        "for b in /vendor/lib/modules /vendor_dlkm/lib/modules /odm/lib/modules /lib/modules; do [ -d \"$b\" ] || continue; find \"$b\" -type f -name '*mali*.ko' 2>/dev/null | head -20 | while read f; do echo MODULE=$f; sha256sum \"$f\" 2>/dev/null; done; done",
